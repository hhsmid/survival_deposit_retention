...
def clean_and_prepare_data(df):
    sampled_ids = df['id_rand'].drop_duplicates().sample(n=209, random_state=1)
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    clean_transaction_df, sampled_ids = clean_and_prepare_data(transaction_df)
    print(f"Cleaning data took: {time.time() - start_time} seconds")

    # Create chunk data and aggregate over id's in parallel
    start_time = time.time()
    aggregated_chunks_df = parallel_aggregate_chunks(clean_transaction_df, '2014-01-01', '2019-01-01')
